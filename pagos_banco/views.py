...
class NotificaView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Logging esencial solamente
            logger.info(
                f"Notifica R4: IP={request.META.get('REMOTE_ADDR')}, IdComercio={id_comercio}, Monto={monto}, Referencia={referencia}"
            )
            ...
            try:
                ...
                if rif_comercio_recibido != rif_comercio_nuestro:
                    logger.warning(
                        f"Notifica R4: [RECHAZADO] IdComercio {id_comercio} no coincide con nuestro RIF {rif_comercio_nuestro}"
                    )
                    print(
                        f"[VALIDACION FALLIDA] IdComercio {id_comercio} no coincide con nuestro RIF {rif_comercio_nuestro}"
                    )
                ...
                if not monto or not referencia or not banco_emisor:
                    logger.warning(
                        f"Notifica R4: [RECHAZADO] Campos requeridos faltantes: Monto={monto}, Referencia={referencia}, BancoEmisor={banco_emisor}"
                    )
                ...
                if not telefono_banco_normalizado:
                    logger.error(
                        f"Notifica R4: [RECHAZADO] Teléfono del emisor es obligatorio para validar el pago. Monto: {monto}"
                    )
                ...
                compras_pendientes_qs = (
                    ...
                    .order_by("-FechaCompra")[:50]
                )  # Limitar a 50 para evitar cargar demasiados
                ...
                if compra:
                    logger.info(
                        f"Notifica R4: [OK] Compra encontrada por monto Y telefono: #{compra.Id}, Tel banco: {telefono_emisor}, Tel compra: {compra.idComprador.NumeroTlf}"
                    )
                else:
                    # CRÍTICO: Si no hay coincidencia de teléfono, RECHAZAR el pago
                    # Esto previene que un pago se asigne al usuario incorrecto
                    logger.error(
                        f"Notifica R4: [RECHAZADO] No se encontro compra pendiente con monto {monto} Y telefono {telefono_emisor}. El teléfono debe coincidir exactamente."
                    )

                    # Log adicional para debugging: mostrar qué compras pendientes hay (ya están en memoria)
                    if compras_pendientes:
                        logger.warning(
                            f"Notifica R4: [DEBUG] Compras pendientes encontradas por monto {monto}:"
                        )
                        for compra_debug in compras_pendientes[:5]:  # Mostrar máximo 5
                            ...
                            logger.warning(
                                f"  - Compra #{compra_debug.Id}: Teléfono {tel_compra} (no coincide con {telefono_emisor})"
                            )
                ...
                if not compra:
                    logger.error(
                        f"Notifica R4: [ERROR CRÍTICO] Compra es None después de validación. Esto no debería ocurrir."
                    )
                    return JsonResponse({"abono": False})

                logger.info(
                    f"Notifica R4: [OK] Compra encontrada: #{compra.Id}, Cliente: {compra.idComprador.Nombre if compra.idComprador else 'N/A'}"
                )
                ...
                if referencia:
                    ...
                    if compra_existente_ref:
                        logger.warning(
                            f"Notifica R4: [RECHAZADO] Referencia {referencia} ya fue usada en compra #{compra_existente_ref.Id}"
                        )
                        return JsonResponse({"abono": False})
                ...
                with transaction.atomic():
                    ...
                    if numeros_compra_lista:
                        ...
                        if numeros_a_agregar:
                            ...
                            rifa.save()
                            logger.info(
                                f"Notifica R4: Actualizados {len(numeros_a_agregar)} numeros en TotalComprados para rifa {rifa.Id}"
                            )
                    ...
                    if numeros_compra_lista:
                        ...
                        if numeros_eliminados[0] > 0:
                            logger.info(
                                f"Notifica R4: {numeros_eliminados[0]} numeros reservados liberados para compra {compra.Id}"
                            )
                ...
                logger.info(
                    f"Notifica R4: [PAGADA] COMPRA #{compra.Id} PAGADA - Cliente: {cliente_nombre}, Monto: {monto}, Ref: {referencia}"
                )
            ...
            if abono_valido:
                try:
                    ...
                    if transacciones_existentes.exists():
                        ...
                        transaccion.save()
                        logger.info(
                            f"Notifica R4: Transacción {transaccion.id} actualizada"
                        )
                    else:
                        ...
                        if compra_actualizada and compra:
                            ...
                            transaccion.save()
                        logger.info(
                            f"Notifica R4: Nueva transaccion {transaccion.id} creada"
                        )
                except Exception as e:
                    logger.error(
                        f"Notifica R4: [ERROR] Error al actualizar/crear transaccion: {str(e)}"
                    )
                    import traceback
