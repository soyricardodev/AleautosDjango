...
def index(request):
    try:
        ...
        Rifas_qs = RifaModel.objects.filter(Estado=True, Eliminada=False).order_by(
            "-Id"
        )
        ...
        numDisp = (
            NumeroRifaDisponibles.objects.filter(idRifa=Rifa).count() if Rifa else 0
        )
        ...
        settings_codes = [
            "PHONE_CLIENT",
            "HIDE_TICKET_COUNT",
            "ZELLE_CONDITION_TYPE",
            "ZELLE_MIN_VALUE",
            "ZELLE_EMAIL",
        ]
        settings_dict = {
            s.code: s for s in Settings.objects.filter(code__in=settings_codes)
        }
...
def Dashboard(request):
    ...
    totalComprasAprobadas = (
        Compra.objects.filter(idRifa=Rifa)
        .filter(Estado=Compra.EstadoCompra.Pagado)
        .count()
    )
    totalComprasRechazadas = (
        Compra.objects.filter(idRifa=Rifa)
        .filter(Estado=Compra.EstadoCompra.Rechazado)
        .count()
    )
    totalComprasPendientes = (
        Compra.objects.filter(idRifa=Rifa)
        .filter(Estado=Compra.EstadoCompra.Pendiente)
        .count()
    )
                ...
                with transaction.atomic():
                    ...
                    user = User.objects.create_user(
                        username=form.cleaned_data[
                            "correo"
                        ],  # Usar correo como username
                        email=form.cleaned_data["correo"],
                        password=form.cleaned_data["password"],
                        first_name=form.cleaned_data["nombre"].split()[0]
                        if form.cleaned_data["nombre"].split()
                        else "",
                        last_name=" ".join(form.cleaned_data["nombre"].split()[1:])
                        if len(form.cleaned_data["nombre"].split()) > 1
                        else "",
                    )
            ...
            if user:
                ...
                if user is not None:
                    ...
                    if hasattr(user, "cliente"):
                        ...
                        error = (
                            "Esta cuenta es de administrador. Usa /Login/ para acceder."
                        )
                else:
...
def mi_perfil(request):
    ...
    try:
        ...
        # Obtener rifa activa
        rifas_activas = RifaModel.objects.filter(Estado=True, Eliminada=False).order_by(
            "-Id"
        )
        ...
        if rifa_activa:
            ...
            for compra in compras:
                numeros_compra = NumerosCompra.objects.filter(
                    idCompra=compra
        ...
        context = {
            ...
            "todos_los_numeros": sorted(
                todos_los_numeros, key=lambda x: int(x) if x.isdigit() else 0
            ),
...
def compradorDialog(request):
    ...
    if "cliente" in data:
        ...
        return JsonResponse(
            {"error": "Se requiere 'comprador' o 'cliente' en el request"}, status=400
        )
...
def dialogSettings(request):
    ...
    zelle_condition_type_config = Settings.objects.filter(
        code="ZELLE_CONDITION_TYPE"
    ).first()
...
def tableDialogBuscaNumero(request, id=None):
    ...
    context = {"Compras": ComprasLista, "compras_paginadas": page_obj}
    return HttpResponse(template.render(context, request))
...
def tableDialog(request, id=None):
    ...
    texto = (
        request.session["textoBusqueda"] if "textoBusqueda" in request.session else None
    )
    ...
    dateInicio = (
        request.session["dateInicio"] if "dateInicio" in request.session else None
    )
    ...
    if texto != None and texto != "":
        ComprasLista = ComprasLista.filter(
            Q(idComprador__Nombre__icontains=texto)
            | Q(idRifa__Nombre__icontains=texto)
            | Q(idComprador__Cedula__icontains=texto)
        )
...
def tableDialogPDF(request, id=None):
    ...
    if tipo == 1:
        logger.info("entro")
        texto = (
            request.session["textoBusqueda"]
            if "textoBusqueda" in request.session
            else None
        )
        estado = request.session["Estado"] if "Estado" in request.session else None
        grupoCookie = request.session["grupo"] if "grupo" in request.session else None
        ...
        dateFinal = (
            request.session["dateFinal"] if "dateFinal" in request.session else None
        )
        """if grupo!= None and grupo!="" and grupo!="0":
            estado=None
            texto=None
        ...
        if grupo != None and grupo != "":
            ...
            if grupo == "1":
                ...
                ComprasLista = (
                    ComprasLista.values("idComprador__Correo")
                    .annotate(
                        head_count=Count("idComprador__Correo"),
                        totalNumeros=Sum("NumeroBoletos"),
                        totalPagadoS=Sum("TotalPagado"),
                    )
                    .values(
                        "idComprador__Correo",
                        "head_count",
            ...
            if grupo == "2":
                ComprasLista = (
                    ComprasLista.values("idComprador__Cedula")
                    .annotate(
                        head_count=Count("idComprador__Cedula"),
                        totalNumeros=Sum("NumeroBoletos"),
                        totalPagadoS=Sum("TotalPagado"),
                    )
                    .values(
            ...
            if grupo == "3":
                ComprasLista = (
                    ComprasLista.values("idComprador__Nombre")
                    .annotate(
                        head_count=Count("idComprador__Nombre"),
                        totalNumeros=Sum("NumeroBoletos"),
                        totalPagadoS=Sum("TotalPagado"),
                    )
                    .values(
    ...
    if tipo == 1 and (grupo == "1" or grupo == "2" or grupo == "3"):
        ...
        for row in rows:
            for numero in row.NumerosCompra.all():
                ...
                string = (
                    row.FechaCompra.strftime("%Y-%m-%d %H:%M:%S")
                    if row.FechaCompra is not None
                    else "   "
                )
                string2 = (
                    row.FechaEstado.strftime("%Y-%m-%d %H:%M:%S")
                    if row.FechaEstado is not None
                    else "   "
                )
                ...
                totalPagado = (
                    f"Bs. {row.TotalPagado}"
                    if row.MetodoPago == 3
                    else f"$ {row.TotalPagadoAlt}"
                )
...
def Historial(request, id=None):
    ...
    RifaHistorial = None
    if (
        "Estado" in request.session
        or "textoBusqueda" in request.session
        or "pay_type" in request.session
        or "dateInicio" in request.session
        or "dateFinal" in request.session
        or "grupo" in request.session
        or "numeroBusquedaTotal" in request.session
    ):
        texto = (
            request.session["textoBusqueda"]
            if "textoBusqueda" in request.session
        ...
        pay_type = (
            int(request.session["pay_type"]) if "pay_type" in request.session else None
        )
        dateInicio = (
            request.session["dateInicio"] if "dateInicio" in request.session else None
        )
        ...
        numeroBusquedaTotal = (
            request.session["numeroBusquedaTotal"]
            if "numeroBusquedaTotal" in request.session
            else None
        )
    ...
    if request.method == "GET":
        ...
        if is_cookie_set == 1:
            ...
            if estado != None and estado != "":
                ...
                if estado == "2":
                    ComprasLista = ComprasLista.filter(
                        Estado=Compra.EstadoCompra.Cancelado
                    )
                if estado == "3":
                    ComprasLista = ComprasLista.filter(
                        Estado=Compra.EstadoCompra.Pagado
                ...
                if estado == "4":
                    ComprasLista = ComprasLista.filter(
                        Estado=Compra.EstadoCompra.Rechazado
                    )
                if estado == "5":
                    ComprasLista = ComprasLista.filter(
                        Estado=Compra.EstadoCompra.Caducado
                    )
            ...
            if texto != None and texto != "":
                ComprasLista = ComprasLista.filter(
                    ...
                    | Q(idComprador__Correo__icontains=texto)
                )
            ...
            if grupo != None and grupo != "":
                ...
                if (
                    grupo != "0"
                    and numeroBusquedaTotal != None
                    and numeroBusquedaTotal != ""
                ):
...
def Usuarios(request):
    ...
    if (
        "textoBusquedaUsuarios" in request.session
        or "dateInicioUsuarios" in request.session
        or "dateFinalUsuarios" in request.session
    ):
    ...
    ClientesLista = (
        Cliente.objects.all().select_related("user").order_by("-fecha_registro")
    )
...
def deleteRifa(request):
    if request.method == "POST":
        ...
        if Rifa.Estado == True:
            return JsonResponse(
                {"status": "error", "message": "No se puede eliminar una rifa activa"}
            )
        ...
        if compras.count() > 0:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "No se puede eliminar una rifa con compras",
                }
            )
        Rifa.Eliminada = True
        Rifa.Estado = False
...
def copyRifa(request):
    if request.method == "POST":
        ...
        rifas_nombres_parecidos = (
            RifaModel.objects.all().filter(Nombre__startswith=Rifa.Nombre).count()
        )
        new_rifa.Nombre = Rifa.Nombre + f"({rifas_nombres_parecidos + 1})"

        ###Para el nombre de enlace hay que hacer un detalle
        rifas_parecidas = (
            RifaModel.objects.all()
            .filter(NombreEnlace__icontains=Rifa.NombreEnlace)
            .count()
        )
        ...
        numDisp = NumeroRifaDisponiblesArray(
            idRifa=new_rifa,
            Numeros=[
                "{:0{}d}".format(number, zerroFill)
                for number in range(
                    new_rifa.RangoInicial, new_rifa.RangoFinal + 1, new_rifa.Intervalo
                )
            ],
        )
        ...
        for i in range(
            ...
            numDispList.append(
                NumeroRifaDisponibles(
                    idRifa=new_rifa, Numero="{:0{}d}".format(i, zerroFill)
                )
            )
            total += 1
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": "Error guardando la rifa"}, status=500
            )
        except Exception as e:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if rifa.DiasFecha == "" or rifa.DiasFecha is None:
                rifa.DiasFecha = 0
            if (
                rifa.FechaSorteo == "" or rifa.FechaSorteo is None
            ) and modo_porcentaje is None:
                rifa.FechaSorteo = datetime.now(
            ...
            numDisp = NumeroRifaDisponiblesArray(
                ...
                Numeros=[
                    "{:0{}d}".format(number, zerroFill)
                    for number in range(
                        rifa.RangoInicial, rifa.RangoFinal + 1, rifa.Intervalo
                    )
                ],
            )
            ...
            for i in range(rifa.RangoInicial, rifa.RangoFinal + 1, rifa.Intervalo):
                numDispList.append(
                    NumeroRifaDisponibles(
                        idRifa=rifa, Numero="{:0{}d}".format(i, zerroFill)
                    )
                )
                total += 1
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if rif.Estado == False and rif.TotalComprados == 0:
                ...
                # Premios
                if (
                    "PremioDescripcion[]" in request.POST
                    and "PremioImagen[]" in request.FILES
                ):
                    descripcionlist = request.POST.getlist("PremioDescripcion[]")
                    imagenlist = request.FILES.getlist("PremioImagen[]")
                    for num in range(len(descripcionlist)):
                        if descripcionlist[num] != "":
                            logger.info(descripcionlist[num])
                            premio_viejo = PremiosRifa.objects.filter(
                                Nombre=descripcionlist[num], idRifa=rifa
                            )
                            if not premio_viejo:
                                premio = PremiosRifa(
                                    idRifa=rifa,
...
def vistaCompra(request, name):
    if request.method == "GET":
        if name == None:
            ...
            try:
                CompraObj = Compra.objects.filter(
                    Estado=Compra.EstadoCompra.Pagado
                ).get(hash=name)
                # array numbers
                arr = [
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if form.is_valid():
                ...
                compras = Compra.objects.filter(idRifa=Rifa).filter(
                    idComprador__Correo=correo
                )
                numerosAprobados = (
                    NumerosCompra.objects.filter(idCompra__idRifa=Rifa)
                    .filter(idCompra__Estado=Compra.EstadoCompra.Pagado)
                    .filter(idCompra__idComprador__Correo=correo)
                )
                numerosPendientes = (
                    NumerosCompra.objects.filter(idCompra__idRifa=Rifa)
                    .filter(idCompra__Estado=Compra.EstadoCompra.Pendiente)
                    .filter(idCompra__idComprador__Correo=correo)
                )
...
def recuperaNumeros2(request):
    ...
    # 35 compras
    compraL = (
        Compra.objects.filter(idRifa=RifaC)
        .filter(Estado=Compra.EstadoCompra.Rechazado)
        .filter(recuperado=False)
    )
...
def recuperaNumeros(request):
    ...
    RifaCompras = Compra.objects.filter(idRifa=RifaC).filter(
        Estado=Compra.EstadoCompra.Pagado
    )
    x = 0
...
def loopReenvio(ReenvioObj, RifaCompras, x):
    for CompraObj in RifaCompras:
        ...
        try:
            ...
            ReenvioObj.save()
            return JsonResponse(
                {"message": f"Error en la solicitud, se enviaron: {x} correos"},
                safe=False,
                status=500,
            )
            ...
            try:
                ...
                for CompraObj in batch_of_RifaCompras:
                    ...
                    texto = (
                        ...
                        + " tu compra ["
                        + str(CompraObj.Id)
                        + "] ha sido aprobada, puedes consultar tus numeros en el siguiente enlace, asegurate de no compartirlo con nadie "
                        + settings.URL
                        + "/Comprobante/"
                        + str(CompraObj.hash)
                    )
                ...
                conn.request(
                    "POST",
                    "/api/Message/AddMessagestoQueue",
                    json.dumps(payload),
                    headers,
                )
...
def reenvioMasivo(request):
    if request.method == "POST":
        ...
        if RifaCompras.count() > 0:
            ...
            ReenvioObj = ReenviosMasivos.objects.filter(idRifa=RifaC, estado=2).last()
            ...
            return JsonResponse(
                {
                    "message": f"Solicitud procesada con éxito, cantidad correos a enviar: {(RifaCompras.count())}"
                },
                safe=False,
                status=200,
            )
...
def reenvioMasivoWS(request):
    if request.method == "POST":
        ...
        if RifaCompras.count() > 0:
            ...
            return JsonResponse(
                {"message": f"Solicitud procesada con éxito"}, safe=False, status=200
            )
...
def CompraRifa(request, name):
    ...
    context = {"id": id, "Rifa": Rifa, "RifaC": RifaC, "tasa": tasa, "numDisp": numDisp}
    return HttpResponse(template.render(context, request))
